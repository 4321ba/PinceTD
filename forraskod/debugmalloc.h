// 1234ab: 2 helyen módosítottam, hogy elnémítsam a figyelmeztetéseket: MODOSITOTT -tal jeleztem
...
int putenv(char const *envvar); // MODOSITOTT hogy elnémítsam a figyelmeztetést, hogy "implicit declaration"
...
static void *debugmalloc_malloc_full(size_t size, char const *func, char const *expr, char const *file, unsigned line, bool zero) {
    ...
    /* check max size */
    DebugmallocData *instance = debugmalloc_singleton();
    if ((long int)size > instance->max_block_size) { // MODOSITOTT: problémázott, hogy különböző előjeltípusúakat hasonlított össze, beleírtam, hogy (long int)
